package com.chart.constant;

import android.os.Environment;

public class GlobConstant {

	public final static String PACKAGE_NAME = Environment
			.getExternalStorageDirectory() + "/" + "com.chart";

	public final static String KEY = "12345678";

	// send keys

	// login type
	public final static String ACCOUNT_LOGIN = "1";

	public final static String DOCUMENT_LOGIN = "2";

	public final static String DATABASE_NAME = "chart.db";

	public final static String DATABASE_DIRECTORY = "database";

	/**
	 * 聊天类型，群聊，个人
	 */
	public final static String GROPU = "0";

	public final static String PERSONAL = "1";

	public final static String UPDATE_RECEIVER = "com.chart.ChartActivity";

	public final static String CHANGE_TO_PERSONAL = "com.chart.changeToPersonal";

//	public final static String SERVER_IP = "192.168.0.100";

	 public final static String SERVER_IP = "172.16.19.70";

	public final static String SERVER_PORT = "9966";

	public final static String PUSH_PORT = "9999";

	public final static String IMAGE_TYPE = "image/*";

	public final static int FROM_ALBUMS = 2;

	public final static int TAKE_PHOTO = 1;

}
